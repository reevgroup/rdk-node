...
describe('auth (browser)', function () {
	...
	test(`sets default auth mode to cookie`, async (url) => {
		const sdk = new RDK(url, { auth: { mode: 'cookie' } });
	...
	test(`logout doesn't send a refresh token due to cookie mode`, async (url, nock) => {
		nock()
			...
			.reply(
				...
				{
					'Set-Cookie': 'rdk_refresh_token=my_refresh_token; Max-Age=604800; Path=/; HttpOnly;',
				}
			);

		nock().post('/auth/logout', {}).matchHeader('cookie', 'rdk_refresh_token=my_refresh_token').reply(200, {
			data: {},
		});
